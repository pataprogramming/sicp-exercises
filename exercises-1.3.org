...
boilerplate needed to make them call each other. For example, the
=sum-of-squares= procedure from an earlier section could be rewritten
simply as:

#+BEGIN_SRC scheme :session 1-3
  (define sum-of-squares (compose + square))
#+END_SRC

This will become more important as the books moves beyond simple
numeric manipulations into operations on complex data structures.
